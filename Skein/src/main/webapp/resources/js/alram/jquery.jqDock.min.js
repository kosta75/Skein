	...
	if (!$.fn.jqDock) {
		var jqDock = function() {
			return {
				...
				useJqLoader : $.browser.opera || $.browser.safari,
				shrinkInterval : 100,
				docks : [],
				X : 0,
				Y : 0,
				...
				initDock : function(id) {
					var ME = this, Dock = this.docks[id], op = Dock.Opts, off = 0, AI = $(
							'a, img', Dock.Menu), i = 0, j, el, wh, acc, upad, opPre95 = ($.browser.opera && (1 * ($.browser.version
							.match(/^(\d+\.\d+)/) || [ 0, 0 ])[1]) < 9.5);
					...
					$.each([ op.vh.lead, op.vh.trail ], function(n, v) {
						Dock.Borders[v] = ME.asNumber(Dock.Yard.css('border'
								+ v + 'Width'))
					});
				...
				deltaXY : function(id) {
					...
					if (Dock.Current !== false) {
						var op = Dock.Opts, el = Dock.Elem[Dock.Current], p = el.Pad[op.vh.lead]
								+ el.Pad[op.vh.trail], off = el.Img.offset();
						Dock.Delta = Math
								.floor((this[op.vh.xy] - off[op.vh.tl])
				...
				setLabel : function(id, label) {
					...
					if (!Dock.Label) {
						Dock.Label = $(
								'<div class="jqDockLabel jqDockMouse'
										+ id
										+ '_00 jqDockLabelImage" style="position:absolute;margin:0px;"></div>')
								.hide().bind(
										'click',
										function() {
											Dock.Elem[Dock.Current].Img
													.trigger('click')
										}).appendTo(Dock.Yard)
					}
					if (label.txt) {
				...
				doLabel : function(id, off) {
					...
					if (Dock.Opts.labels && Dock.Current !== false) {
						var el = Dock.Elem[Dock.Current], L = el.Label, op = Dock.Opts, what = typeof off == 'string' ? off
								: 'move';
						switch (what) {
						case 'show':
						case 'hide':
						...
						case 'change':
							Dock.Label[0].className = Dock.Label[0].className
									.replace(/(jqDockLabel)(Link|Image)/, '$1'
											+ (el.Linked ? 'Link' : 'Image'));
						...
						default:
							...
							if (splt[0] == 'm') {
								css.top += Math.floor((el[op.vh.inv.act]
										- L.height - L.heightPad) / 2)
							} else if (splt[0] == 'b') {
								css.top += el[op.vh.inv.act] + el.Pad.Top
										+ el.Pad.Bottom - L.height
										- L.heightPad
							}
							if (splt[1] == 'c') {
								css.left += Math
				...
				mouseHandler : function(e) {
					var r = null, t = e.target.className
							.match(/jqDockMouse(\d+)_(\d+)/), rt = !!(e.relatedTarget)
							&& e.relatedTarget.tagName !== undefined;
					if (t) {
						r = false;
						var id = 1 * t[1], Dock = this.docks[id], idx = t[2] == '00' ? Dock.Current
								: 1 * t[2];
						this.X = e.pageX;
						this.Y = e.pageY;
						if (e.type == 'mousemove') {
				...
				factorSizes : function(id) {
					...
					if (lapse > 60) {
						...
						while (i < Dock.Elem.length) {
							el = Dock.Elem[i];
							this.changeSize(id, i++,
									(f ? Math.floor(el.Initial
											+ ((el.Final - el.Initial) * f))
											: el.Final))
						}
					}
				},
				changeSize : function(id, idx, dim, force) {
					...
					if (force || el.Actual != dim) {
						var op = Dock.Opts, bdr = ($.boxModel || op.orient.vh == 'v') ? 0
						...
						var css = this.keepProportion(el, dim, op.orient), diff = op.size
								- css[op.vh.inv.wh], m = 'margin', z = op.vh.inv;
						switch (op.align) {
						case 'bottom':
						case 'right':
							css[m + z.lead] = diff;
						...
						el.Img.css(css);
						Dock.Yard.css('margin' + op.vh.lead, Math.floor(Math
								.max(0, (Dock[op.vh.wh] - Dock.Spread) / 2)));
						el.Actual = dim;
							...
							function() {
								...
								$('img', this)
										.each(
												function(n) {
													var me = $(this), s0 = me
															.attr('src'), s1 = (op.source ? op.source
															.call(me[0], n)
															: false)
															|| jqDock.altImage
																	.call(this)
