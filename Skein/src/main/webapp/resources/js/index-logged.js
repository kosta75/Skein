$(document).ready(function() {
					...
					$.getJSON("board/getBoardGroup",function(data) {
					 ...
					 $.each(obj, function(key, value) {var board = value;
											source.push([ value.groupStartDate,
													value.groupCount ])
											flagSource.push({
																	...
																	},
																	{
																		...
																		events : {
																			click : function(event) {
																				 ...
																				 * try{
																				 * posY =
																				 * document.getElementById(""+target_id).offsetTop;
																				 * }catch(e){
					...
					$(document).on('click','.delete-button',function() {
								...
								var index = $(this).parents('#file-list').find(
										'li').index(li);
					...
					function groupTemplate(groupID, files) {
						...
						for (var i = 0; i < files.length; i++) {
							...
							html.push("<li><div id='"
											...
											+ "' data-groupid='"
											+ groupID
											+ "' class='image-item'><div class='delete-button'></div></div></li>");
						}
						var start = "<li>";
					...
					var opts = {
						on : {load : function(e, file) {
								...
								if (file.type.match(/image/)) {
									...
									// fileDiv.find(".modal").append($("<div
									// />").text(e.target.result));
								}
							},
							error : function(e, file) {
								$("#group_" + file.extra.groupID
												+ "_file_" + file.extra.fileID).addClass("error");
							},
							groupstart : function(group) {
								$(filelist).append(groupTemplate(group.groupID,group.files));
								/*
								 * $(filelist).find( ".details
								 * a:not(.initialized)") .click(function() {
								 ...
								 * show : false }).on("show", function() {
								 * console.log("여기에 뭘 쓰지?"); });
								 */
							},
							groupend : function(group) {
								/*
								 * $("#group_" + group.groupID).append( "<div
								 * style='display:none;'>(Time to load: " +
								 * (group.ended - group.started) + "ms)</div>");
								 */
							}
						}
					};
					...
					/*
					 * $(document).on( 'click', '.members', function() { var
					 * result = confirm($(this).data("name") + "(" +
					 * $(this).data("uri") + ") 님께 친구를 신청하시겠습니까?"); if (result) {
					 * $.ajax({ type : 'POST', url : 'friendship/add/' + $(
					 * this).data("uri"), success : function(data) {
					 * alert(data.result); }, error : function() { alert('Error
					 * while request..'); } }); } else { console.log("친구 추가취소"); }
					 * });
					 * 
					 * $('#search .textbox').on('keyup', function() { if
					 * ($("#search .textbox").val().length > 0) { $.ajax({ type :
					 * 'POST', url : 'search/members', data : "fullName=" +
					 * $("#search .textbox").val(), success : function(data) {
					 * console.log(data); console.log(data.list); var obj =
					 * data.list; var msg = "";
					 * 
					 * for (var i = 0; i < obj.length; i++) { msg += "<div
					 * class='members' data-name='" + obj[i].fullName + "'
					 * data-uri='" + obj[i].personalURI + "'>" + obj[i].fullName +
					 * "/" + obj[i].personalURI + "</div>"; }
					 * 
					 * $("#searchMembers").html(msg); }, error : function() {
											...
											$.ajax({
														...
														success : function(data) {
															...
															if (data.result > 0) {
																...
																replyeditform.siblings().first().next().prepend(dddreplyeditBtn);
					...
					$("#historyWriteMenu3").click(function() {
										...
										if ($("#historymap").css("display") == "block") {
											$("#historymap").css("display",
													"none");
										} else {
											$("#historyImg").css("display",
													"none");
											$("#historyDate").css("display",
													"none");
											$("#historymap").css("display",
													"block");
											$("#historyemoticon").css(
					...
					$(document).on("click",".edit-emoticon-icon",function() {
						$(this).parent().siblings().first().append(
										"<img class='emoticonImg' src="
												+ $(this).attr("src") + ">");
					...
					$(document).on("click",".reply-show-hide-btn",function() {
								if ($(this).parent().siblings().next().next()
										.next().css("display") == "none") {
									$(this).parent().siblings().next().next()
											.next().show("slide", {
												direction : "up"
											});
								} else {
					...
					$(document).on('click',".group-item-delete",function() {
										...
										var groupCount = $(this).data(
												"groupcount");
										var result = confirm(groupCount
												+ "개의 사진이 존재합니다 모두 삭제 하시겠습니까?");
					...
					$(document).on('click','.group-item-edit',function() {
								...
								editcontent.siblings().first().css("display",
										"block");
							});
					...
					$(document).on('click','.edit-emoticon',function() {
								...
								if ($(this).parent().siblings().first().next()
										.next().css('display') == "none") {

									$(this).parent().siblings().first().next()
										...
										$(this)
												...
												.val(
														$(this).parent()
																.siblings()
																.first().html());
										var editContent = $(this).parent()
												.siblings().first().next()
												.val();
										var boardSeq = $(this).parent()
												.siblings().first().next()
												.data("boardseq");
										var publicLevelCode = $(
												".publicLevelCode").val();
					...
					$(document).on("click",".modal-Edit",function() {
								...
								$(this).parent().parent().siblings().first()
										.css("display", "block");
								$(this).parent().parent().siblings().first()
										.children().first().html(
					...
					$(document).on("click",".modaleditCancleBtn",function() {
								...
								$(this).parent().siblings().first().next().css(
										"display", "block");
					...
					$(document).on("click", ".modaleditEmoticon", function() {
						if ($("#modalemoticon").css("display") == "none") {
							$("#modalemoticon").show("slide", {
								direction : "left"
							});
						} else {
							$("#modalemoticon").hide("slide", {
								direction : "left"
							});
						}
					...
					$(document).on("click",".modaleditBtn",function() {
										...
										$(this).siblings().first().next().val(
												$(this).siblings().first()
														.html());
										var editContent = $(this).siblings()
												.first().next().val();
										var publicLevelCode = $(
												".publicLevelCode").val();
					...
					//submit
					$("#hitstoryWriteBtn").click(
							function() {
								$("#content").val($("#writeTextarea").html());
								$("#historyhiddenplace").val(
										$("#historyplace").text());
								if ($("#content").val() != null
										&& $("#content").val() != '') {
									$("#hitstoryForm").submit();
					...
					$(document).on('click','.share-btn',function() {
										...
										parents.find('.share-info-div').show("slide",{direction : "left"},
														function() {
															$.ajax({
																		type : 'post',
																		url : 'board/shareView',
																		...
																		success : function(data) {
																			...
																			if (boardsharedetail == 0|| boardsharedetail == 1) {
																				parents.find(".share-img-list").append(
																								"<div class='share-data-boardSeq' data-boardSeq='"
																										+ data.boardshare[0].boardSeq
																										+ "' style='background-color : white;border-radius:10px 10px 10px 10px;'>"
																										+ "<div style='float:left;'>"
																										+ "<input type='checkbox' checked disabled name='shareCheckBoxGroup' value="
																										+ data.boardshare[0].boardSeq
																										+ " data-boardSeq="
																										+ data.boardshare[0].boardSeq
																										+ "></div>"
																				...
																				for (var j = 0; j < boardsharedetail; j++) {
																					parents.find(".share-img-list").append(
																									"<div class='share-data-boardSeq' data-boardSeq='"
																											+ data.boardshare[j].boardSeq
																											+ "'style='background-color : white;border-radius:10px 10px 10px 10px;' >"
																											+ "<div style='float:left;margin-top:10px'>"
																											+ "<input type='checkbox' name='shareCheckBoxGroup' value="
																											+ data.boardshare[j].boardSeq
																											+ " data-boardSeq="
																											+ data.boardshare[j].boardSeq
																											+ "></div>"
																											...
																											+ j
																											+ "'>"
																											+ "<img class='shareimgBtn' data-imgBtnNumber = '"
																											+ j
																											+ "'  src='./resources/upload/"
																											+ data.boardshare[j].filename
					...
					$(document).on(
							...
							'.shareimgBtn',
							function() {
								$('#share-detail-preview').css("display",
										"block");
								$('#share-detail-preview-imgtag').attr("src",
										$(this).attr("src"));
							});
					...
					$(document).on('click','.detailImg',	function() {
										...
										$('#modal-content').append(
														"<div id='modal-detile-view'style='width: 960px; height: 540px; display: block;z-index:1000;position:absolute;'>	<img style='z-index:10;width: 960px; height: 540px; ' src='"
																+ $(this).find("img").attr("src")+ "'></div>");
									});
					...
					$(document).on('click','#share-confirm-btn',function() {
										...
										$(document)
												.find(
														"input[name=shareCheckBoxGroup]:checked")
												.each(function(i, j) {
													checked[i] = j.value;
												});
										//if(chked_val!="")chked_val = chked_val.substring(1);
										var checkedLength = $(document)
												.find(
														"input[name=shareCheckBoxGroup]:checked").length;
					...
					function get_chked_values() {
						var chked_val = "";
						$(document).find(
								"input[name=shareCheckBoxGroup]:checked").each(
								function(pi, po) {
									chked_val += "," + po.value;
								});
						if (chked_val != "")
					...
					$(window).scroll(function() {offset = menuYloc + $(document).scrollTop()
										+ "px";
								$(name).animate({
									top : offset
								}, {
